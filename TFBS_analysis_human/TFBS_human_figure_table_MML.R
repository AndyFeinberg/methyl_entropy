...
##get motif order based on the row mean difference between non-regulatory and regulatory DNA regions
motif_MML_mean_DNase_combine_ave <- rowMeans(data_median_com_DNase, na.rm=TRUE)
motif_MML_mean_control_combine_ave <- rowMeans(data_median_com_control, na.rm=TRUE)

motif_order <- sort(motif_MML_mean_control_combine_ave - motif_MML_mean_DNase_combine_ave, decreasing = TRUE, index.return=TRUE)$x %>% names()

##standardize the MML values across regulatory and non-regulatory DNA regions for each motif
...
ht <- Heatmap(data_median_com_filter_sd, name = "MML",
							cluster_rows = FALSE,
...
ht <- Heatmap(data_median_com_filter_top50_sd, name = "MML",
							cluster_rows = FALSE,
							cluster_columns = FALSE,
							column_split = factor(region_group,levels=c("Non-regulatory DNA","Regulatory DNA")),
							column_gap = unit(5, "mm"),
...
data_test_p <- lapply(colnames(data_all[,-1]), function(t){
  data_sub <- data_all[,c("group",t)]
	colnames(data_sub) <- c("group","MML")
	p_value <- wilcox.test(data_sub[data_sub$group=="Non-regulatory DNA","MML"],data_sub[data_sub$group=="Regulatory DNA","MML"],paired = TRUE)$p.value
})
...
plot_list <- lapply(colnames(data_all_plot[,-1]), function(t){
  ...
  data_sub <- data_all_plot[,c("group",t)]
	...
	p <- ggplot(data_sub, aes(x=group, y=MML, fill=group)) +
