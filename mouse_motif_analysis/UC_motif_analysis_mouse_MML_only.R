...
##calculate motif enrichment for MML only regions
motif_enrichment_tissue <- mclapply(tissue_name, function(x){

	data_test <- cluster_gr_anno[[x]]
	target <- data_test[data_test$region_type == "MML only"]
	...
	saveRDS(motif_enrichment, file=glue("./enhancer_region/{x}_motif_enrichment_DNase_control_dMML.rds"))
},mc.cores=4)
